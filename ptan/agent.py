...
def default_states_preprocessor(states):
    ...
    if len(states) == 1:
        ...
        np_states = np.array([np.array(s[0], copy=False) if isinstance(s,tuple) else np.array(s, copy=False) for s in states], copy=False)
        
    #print("default_states_preprocessor(): np_states.shape: " + str(np_states.shape))
    #print("default_states_preprocessor(): np_states.dtype: " + str(np_states.dtype))
...
class ActorCriticAgent(BaseAgent):
    ...
    def __call__(self, states, agent_states=None):
        ...
        :return: list of actions
        """
        #print("ActorCriticAgent.__call__()")
        if self.preprocessor is not None:
            states = self.preprocessor(states)
            if torch.is_tensor(states):
                states = states.to(self.device)
