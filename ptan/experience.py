...
class ExperienceSource:
    ...
    def __init__(self, env, agent, steps_count=2, steps_delta=1, vectorized=False):
        ...
        self.storing_folder = "training_ExperienceSourceRollouts"
        self.experience_folder = "experience"
        if os.path.exists(self.storing_folder):
            rmtree(self.storing_folder)
            rmtree(self.experience_folder)
        os.makedirs(self.storing_folder)
        os.makedirs(self.experience_folder)
        ...
        while True:
            ...
            for env_idx, (env, action_n) in enumerate(zip(self.pool, grouped_actions)):
                ...
                for ofs, (action, next_state, r, is_done) in enumerate(zip(action_n, next_state_n, r_n, is_done_n)):
                    ...
                    if is_done:
                        storing_path = os.path.join(self.storing_folder,"iter_idx_{}_env_idx_{}_steps_taken_{}_reward_{:.3f}.gif".format(iter_idx,env_idx,env.steps_taken, cur_rewards[idx]))
                        env.store_frames_in_gif(filename=storing_path)
                        if cur_rewards[idx] > self.best_reward:
                            self.best_reward = cur_rewards[idx]
                            env.store_frames_in_gif(filename="experience/game_best_{:.1f}_iter_idx_{}_env_idx_{}_steps_taken_{}.gif".format(self.best_reward, iter_idx, env_idx,env.steps_taken))
                        if cur_rewards[idx] < self.worst_reward:
                            self.worst_reward = cur_rewards[idx]
                            env.store_frames_in_gif(filename="experience/game_worst_{:.1f}_iter_idx_{}_env_idx_{}_steps_taken_{}.gif".format(self.worst_reward, iter_idx, env_idx, env.steps_taken))
                        #if total_rewards[env_idx] > self.best_reward:
                        #    self.best_reward = total_rewards[env_idx]
                        #    e.store_frames_in_gif(filename="experience/game_best_{:.3f}_all_steps_{}_env_idx_{}_steps_taken_{}.gif".format(self.best_reward, all_steps, env_idx, e.steps_taken))
                        #if total_rewards[env_idx] < self.worst_reward:
                        #    self.worst_reward = total_rewards[env_idx]
                        #    e.store_frames_in_gif(filename="experience/game_worst_{:.3f}_all_steps_{}_env_idx_{}_steps_taken_{}.gif".format(self.worst_reward, all_steps, env_idx,e.steps_taken))
...
class ExperienceSourceRollouts:
    ...
    def __iter__(self):
        ...
        mb_dones = np.zeros((pool_size, self.steps_count), dtype=bool)
        total_rewards = [0.0] * pool_size
        total_steps = [0] * pool_size
        agent_states = None
        step_idx = 0
        all_steps = 0
        storing_folder = "training_ExperienceSourceRollouts"
        experience_folder = "experience"
        if os.path.exists(storing_folder):
            rmtree(storing_folder)
            rmtree(experience_folder)
        ...
        while True:
            ...
            for env_idx, (e, action) in enumerate(zip(self.pool, actions)):
                ...
                if done:
                    if all_steps % 500 == 0:
                        storing_path = os.path.join(storing_folder,"all_steps_{}_env_idx_{}_steps_taken_{}_reward_{:.3f}.gif".format(all_steps,env_idx,e.steps_taken, total_rewards[env_idx]))
                        e.store_frames_in_gif(filename=storing_path)
                    if total_rewards[env_idx] > self.best_reward:
                        self.best_reward = total_rewards[env_idx]
                        e.store_frames_in_gif(filename="experience/game_best_{:.3f}_all_steps_{}_env_idx_{}_steps_taken_{}.gif".format(self.best_reward, all_steps, env_idx, e.steps_taken))
                    if total_rewards[env_idx] < self.worst_reward:
            ...
            #print("step_idx: " + str(step_idx))
            #print("self.steps_count: " + str(self.steps_count))
            if step_idx == self.steps_count:
                ...
                #print("mb_states.shape: " + str(mb_states.shape))
                #new_states =mb_states.reshape((-1,) + mb_states.shape[2:])
                #print("new_states.shape: " + str(new_states.shape))
                #exit()
                #storing_path = os.path.join(storing_folder,"{}.gif".format(all_steps))
                #self.pool[env_idx].store_frames_in_gif(storing_path)
                #exit()
